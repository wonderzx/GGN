...
print("Data Loading...")
# get data
train_data_loader, valid_data_loader, test_data_loader, edges_train = load_bn_ggn(batch_size=Batch_Size,dyn_type=Dyn_Type, adj_address = './data/bn/mark-13826-adjmat.pickle', series_address = './bn_json/bn.pickle')
    ...
    for i in range(Net_Steps):
        ...
        for batch_idx, (data_train, data_target) in enumerate(train_data_loader):
            if use_cuda:
                ...
                data_target = data_target.cuda()
            loss, out_matrix = train_batch_generator(optimizer_network, gumbel_generator, dyn_learner, data_train,
                                                     data_target, loss_fn)
            record_loss = loss.data.tolist()
